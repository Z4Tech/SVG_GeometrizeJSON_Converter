...
def draw_memory_shape(pid: int, shape: Shape, index: int, cLiveryLayerTable: int, liveryCount: int):
    ...
    elif shape.type_id == 99:
        shape_id_data = struct.pack('B', 103)
        write_process_memory(pid, current_layer_address + 0x7A, shape_id_data)
        skew_data = struct.pack('f', shape.skew)
        write_process_memory(pid, current_layer_address + 0x70, skew_data)
...
def main(args):
    ...
    with open(path) as f:
        ...
        try:
            ...
            #     print(len(data['shapes'][0]['data']))
            #     print(valid)
            #     valid = valid and data['shapes'][0]['type'] > 0
            #     print(data['shapes'][0]['color'])
            #     print(valid)
            #     valid = valid and len(data['shapes'][0]['color']) == 4
            #     print(valid)
            #     if not valid:
            #         print("Tried, Not a valid Geometrize geometry export .json file")
            #         return
            # except:
    ...
    for shape in data['shapes'][1:]:
        ...
        if shape['type'] == 99:
            # Rotated ellipsis
            x,y,w,h,rot_deg,skew = shape['data']
            print(shape['data'])
            r,g,b,a = shape['color']
            print(shape['color'])
            shapes.append(Shape(shape['type'], x, y, w, h, rot_deg,  Color(r,g,b,a), False, skew))
        elif shape['type'] == 16:
            # Rotated ellipsis
